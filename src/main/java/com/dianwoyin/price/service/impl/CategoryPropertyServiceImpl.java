...
//        }
        ...
        List<CategoryPropListItem> categoryPropRespons = PriceBeanUtils.copyProperty(categoryProps, CategoryPropListItem.class);
        if (CollectionUtils.isEmpty(categoryPropRespons)) {
            return null;
        }

        // 查询属性下的所有属性值
        List<Integer> propIds = categoryPropRespons.stream().map(CategoryPropListItem::getId).collect(Collectors.toList());
